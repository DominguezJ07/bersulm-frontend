...
import { CalendarDays, Scissors, Gift, Star, Heart } from 'lucide-react'
    ...
    <main className="bg-[var(--bg-primary)] text-[var(--text-primary)]">
      ...
      >
        ...
        <div className={`relative mx-auto flex min-h-[80vh] max-w-6xl flex-col items-center justify-center px-6 py-24 transition-opacity duration-500 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
          <p className="mb-4 text-sm uppercase tracking-[0.35em] text-[var(--accent)]">BERSULM</p>
          ...
          <div className="mt-10 flex flex-col items-center gap-4 sm:flex-row">
            <button className="inline-flex items-center gap-3 rounded-full bg-[var(--accent)] px-8 py-4 text-[var(--text-primary)] transition hover:bg-[#d18d14]">
              <CalendarDays size={18} /> Reservar cita
            </button>
            <button className="inline-flex items-center gap-3 rounded-full border border-[var(--border-color)] bg-[var(--bg-secondary)] px-8 py-4 text-[var(--text-secondary)] transition hover:border-[var(--accent)] hover:text-[var(--text-primary)]">
              Ver servicios
            </button>
          </div>
        </div>
      ...
      <section className="border-y border-[var(--accent)] bg-[var(--bg-secondary)] py-20">
        <div className="mx-auto grid max-w-6xl gap-8 px-6 sm:grid-cols-3">
          {stats.map((item) => (
            <div key={item.label} className="space-y-3 rounded-[32px] bg-[var(--bg-tertiary)] p-8 text-center shadow-xl shadow-black/10">
              <p className="text-5xl font-bold text-[var(--accent)]">{item.value}</p>
      ...
      <section className="py-20">
        <div className="mx-auto max-w-6xl px-6">
          <div className="mb-10 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.35em] text-[var(--accent)]">Descubre</p>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {features.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="rounded-[32px] border border-[var(--border-color)] bg-[var(--bg-secondary)] p-8 shadow-xl shadow-black/5 transition hover:-translate-y-1">
                <div className="mb-6 inline-flex h-14 w-14 items-center justify-center rounded-3xl bg-[var(--bg-tertiary)] text-[var(--accent)]">
                  <Icon size={24} />
                </div>
      ...
      <section className="bg-[var(--bg-secondary)] py-20">
        <div className="mx-auto max-w-6xl px-6">
          <div className="mb-10 text-center">
            <p className="text-sm uppercase tracking-[0.35em] text-[var(--accent)]">Galería</p>
          ...
          <div className="mb-8 flex flex-wrap items-center gap-3">
            {['todos', 'cortes', 'barba'].map((value) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-semibold transition ${galleryFilter === value ? 'bg-[var(--accent)] text-[var(--text-primary)]' : 'border border-[var(--border-color)] bg-[var(--bg-tertiary)] text-[var(--text-secondary)] hover:border-[var(--accent)] hover:text-[var(--text-primary)]'}`}
              >
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="grid gap-8 lg:grid-cols-2">
            <div className="rounded-[32px] border border-[var(--border-color)] bg-[var(--bg-secondary)] p-10 shadow-xl shadow-black/5">
              <p className="text-sm uppercase tracking-[0.35em] text-[var(--accent)]">Clientes</p>
              <h2 className="mt-3 text-3xl font-semibold">Testimonios que hablan por sí solos</h2>
              <p className="mt-4 text-[var(--text-secondary)]">Cortes impecables, servicio atento y resultados fuera de serie. Nuestro estilo se refleja en cada cliente.</p>
            </div>
            <div className="grid gap-4">
              {['Excelente servicio y trato', 'Ambiente profesional y cómodo', 'Siempre vuelvo por su calidad'].map((text) => (
                <div key={text} className="rounded-[32px] bg-[var(--bg-tertiary)] p-6">
                  ...
                  <p className="mt-3 text-sm text-[var(--text-secondary)]">Servicio rápido, resultados consistentes y atención cercana desde la reserva hasta el final.</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="bg-[var(--bg-secondary)] py-20">
        <div className="mx-auto max-w-6xl px-6 text-center">
          <p className="text-sm uppercase tracking-[0.35em] text-[var(--accent)]">Listado de preguntas</p>
          <h2 className="mt-3 text-3xl font-semibold">Preguntas frecuentes</h2>
          <div className="mt-8 space-y-4">
            {faqs.map((faq, idx) => (
              <div key={faq.q} className="overflow-hidden rounded-[32px] border border-[var(--border-color)] bg-[var(--bg-tertiary)]">
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="text-[var(--accent)]">{faqOpen === idx ? '−' : '+'}</span>
                </button>
                <div className={`overflow-hidden px-6 transition-[max-height] duration-300 ${faqOpen === idx ? 'max-h-40 py-4' : 'max-h-0 py-0'}`}>
